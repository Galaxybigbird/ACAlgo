//+------------------------------------------------------------------+
//|                       ACBreakRevertPro_Stage2_Cluster.mq5        |
//|  Stage-2 portfolio selector for BreakRevertPro optimisation      |
//|  passes using the ClusteringLib database.                        |
//+------------------------------------------------------------------+
...
int   g_selectedClusters[];
SPassRecord g_selected[32];
int   g_selectedTotal = 0;

bool ClusterAlreadyUsed(const int cluster)
{
   for(int i = 0; i < ArraySize(g_selectedClusters); ++i)
   {
      if(g_selectedClusters[i] == cluster)
         return true;
   }
   return false;
}
...
{
   int idx = ArraySize(g_selectedClusters);
   ArrayResize(g_selectedClusters, idx + 1);
   g_selectedClusters[idx] = cluster;
}
...
int LoadSelectedPasses(SPassRecord &buffer[])
{
   ...
   ArrayResize(g_selectedClusters, 0);
   g_selectedTotal = 0;
...
void PresentSelection(const SPassRecord &records[], const int count, const double score)
{
   ...
   string lines = StringFormat("BreakRevertPro Stage-2 (size=%d, score=%.4f)\n", count, score);
