...
private:
   ...
   double m_tf5_sum_pv;          // Sum of price * volume for Timeframe 5
   double m_tf5_sum_v;           // Sum of volume for Timeframe 5
   datetime m_last_tf5_time;     // Last processed time for Timeframe 5
   double m_tf6_sum_pv;          // Sum of price * volume for Timeframe 6
   double m_tf6_sum_v;           // Sum of volume for Timeframe 6
   datetime m_last_tf6_time;     // Last processed time for Timeframe 6
   double m_tf7_sum_pv;          // Sum of price * volume for Timeframe 7
   double m_tf7_sum_v;           // Sum of volume for Timeframe 7
   datetime m_last_tf7_time;     // Last processed time for Timeframe 7
   double m_tf8_sum_pv;          // Sum of price * volume for Timeframe 8
   double m_tf8_sum_v;           // Sum of volume for Timeframe 8
   datetime m_last_tf8_time;     // Last processed time for Timeframe 8
   ...
   ENUM_TIMEFRAMES m_timeframe5; // Fifth additional timeframe
   ENUM_TIMEFRAMES m_timeframe6; // Sixth additional timeframe
   ENUM_TIMEFRAMES m_timeframe7; // Seventh additional timeframe
   ENUM_TIMEFRAMES m_timeframe8; // Eighth additional timeframe
...
public:
   ...
   CVWAPIndicator() 
   {
      ...
      m_timeframe5 = PERIOD_CURRENT;
      m_timeframe6 = PERIOD_CURRENT;
      m_timeframe7 = PERIOD_CURRENT;
      m_timeframe8 = PERIOD_CURRENT;
   ...
   void Init(PRICE_TYPE priceType, 
             ...
             ENUM_TIMEFRAMES timeframe6,
             ENUM_TIMEFRAMES timeframe7,
             ENUM_TIMEFRAMES timeframe8,
             bool tickPrecision)
   {
      ...
      m_timeframe5 = timeframe5;
      m_timeframe6 = timeframe6;
      m_timeframe7 = timeframe7;
      m_timeframe8 = timeframe8;
   ...
   void CalculateOnBar(const MqlRates &rates[], int rates_count,
                      ...
                      double &vwapTF5Buffer[],
                      double &vwapTF6Buffer[],
                      double &vwapTF7Buffer[],
                      double &vwapTF8Buffer[])
   {
      ...
      for(int i = rates_count - 1; i >= 0; i--)
      {
         ...
         // Calculate Timeframe 5 VWAP if enabled
         if(m_timeframe5 != PERIOD_CURRENT)
         {
            datetime current_time = CreateDateTime(rates[i].time, m_timeframe5);
            if(current_time != m_last_tf5_time && m_last_tf5_time != 0)
            {
               m_tf5_sum_pv = 0;
               m_tf5_sum_v = 0;
            }
            m_last_tf5_time = current_time;
            m_tf5_sum_pv += pv;
            m_tf5_sum_v += (double)rates[i].tick_volume;
            if(m_tf5_sum_v > 0)
               vwapTF5Buffer[i] = m_tf5_sum_pv / m_tf5_sum_v;
            else
               vwapTF5Buffer[i] = price;
         }

         // Calculate Timeframe 6 VWAP if enabled
         if(m_timeframe6 != PERIOD_CURRENT)
         {
            datetime current_time = CreateDateTime(rates[i].time, m_timeframe6);
            if(current_time != m_last_tf6_time && m_last_tf6_time != 0)
            {
               m_tf6_sum_pv = 0;
               m_tf6_sum_v = 0;
            }
            m_last_tf6_time = current_time;
            m_tf6_sum_pv += pv;
            m_tf6_sum_v += (double)rates[i].tick_volume;
            if(m_tf6_sum_v > 0)
               vwapTF6Buffer[i] = m_tf6_sum_pv / m_tf6_sum_v;
            else
               vwapTF6Buffer[i] = price;
         }

         // Calculate Timeframe 7 VWAP if enabled
         if(m_timeframe7 != PERIOD_CURRENT)
         {
            datetime current_time = CreateDateTime(rates[i].time, m_timeframe7);
            if(current_time != m_last_tf7_time && m_last_tf7_time != 0)
            {
               m_tf7_sum_pv = 0;
               m_tf7_sum_v = 0;
            }
            m_last_tf7_time = current_time;
            m_tf7_sum_pv += pv;
            m_tf7_sum_v += (double)rates[i].tick_volume;
            if(m_tf7_sum_v > 0)
               vwapTF7Buffer[i] = m_tf7_sum_pv / m_tf7_sum_v;
            else
               vwapTF7Buffer[i] = price;
         }

         // Calculate Timeframe 8 VWAP if enabled
         if(m_timeframe8 != PERIOD_CURRENT)
         {
            datetime current_time = CreateDateTime(rates[i].time, m_timeframe8);
            if(current_time != m_last_tf8_time && m_last_tf8_time != 0)
            {
               m_tf8_sum_pv = 0;
               m_tf8_sum_v = 0;
            }
            m_last_tf8_time = current_time;
            m_tf8_sum_pv += pv;
            m_tf8_sum_v += (double)rates[i].tick_volume;
            if(m_tf8_sum_v > 0)
               vwapTF8Buffer[i] = m_tf8_sum_pv / m_tf8_sum_v;
            else
               vwapTF8Buffer[i] = price;
         }
      }
   }
                       ...
                       double &vwapTF8Buffer[])
   {
      ...
      for(int i = data_count - 1; i >= 0; i--)
      {
         ...
         // Timeframe 5 VWAP calculation
         if(m_timeframe5 != PERIOD_CURRENT)
         {
            datetime current_time = CreateDateTime(time[i], m_timeframe5);
            if(current_time != m_last_tf5_time && m_last_tf5_time != 0)
            {
               m_tf5_sum_pv = 0;
               m_tf5_sum_v = 0;
            ...
            m_last_tf5_time = current_time;
            m_tf5_sum_pv += pv;
            m_tf5_sum_v += (double)volume[i];
            if(m_tf5_sum_v > 0)
               vwapTF5Buffer[i] = m_tf5_sum_pv / m_tf5_sum_v;
            else
               vwapTF5Buffer[i] = price[i];
         }

         // Timeframe 6 VWAP calculation
         if(m_timeframe6 != PERIOD_CURRENT)
         {
            datetime current_time = CreateDateTime(time[i], m_timeframe6);
            if(current_time != m_last_tf6_time && m_last_tf6_time != 0)
            {
               m_tf6_sum_pv = 0;
               m_tf6_sum_v = 0;
            ...
            m_last_tf6_time = current_time;
            m_tf6_sum_pv += pv;
            m_tf6_sum_v += (double)volume[i];
            if(m_tf6_sum_v > 0)
               vwapTF6Buffer[i] = m_tf6_sum_pv / m_tf6_sum_v;
            else
               vwapTF6Buffer[i] = price[i];
         }

         // Timeframe 7 VWAP calculation
         if(m_timeframe7 != PERIOD_CURRENT)
         {
            datetime current_time = CreateDateTime(time[i], m_timeframe7);
            if(current_time != m_last_tf7_time && m_last_tf7_time != 0)
            {
               m_tf7_sum_pv = 0;
               m_tf7_sum_v = 0;
            ...
            m_last_tf7_time = current_time;
            m_tf7_sum_pv += pv;
            m_tf7_sum_v += (double)volume[i];
            if(m_tf7_sum_v > 0)
               vwapTF7Buffer[i] = m_tf7_sum_pv / m_tf7_sum_v;
            else
               vwapTF7Buffer[i] = price[i];
         }

         // Timeframe 8 VWAP calculation
         if(m_timeframe8 != PERIOD_CURRENT)
         {
            datetime current_time = CreateDateTime(time[i], m_timeframe8);
            if(current_time != m_last_tf8_time && m_last_tf8_time != 0)
            {
               m_tf8_sum_pv = 0;
               m_tf8_sum_v = 0;
            ...
            m_last_tf8_time = current_time;
            m_tf8_sum_pv += pv;
            m_tf8_sum_v += (double)volume[i];
            if(m_tf8_sum_v > 0)
               vwapTF8Buffer[i] = m_tf8_sum_pv / m_tf8_sum_v;
            else
               vwapTF8Buffer[i] = price[i];
         }
      }
   }
   ...
   ENUM_TIMEFRAMES GetTimeframe5()
   {
      return m_timeframe5;
   }

   ENUM_TIMEFRAMES GetTimeframe6()
   {
      return m_timeframe6;
   }

   ENUM_TIMEFRAMES GetTimeframe7()
   {
      return m_timeframe7;
   }

   ENUM_TIMEFRAMES GetTimeframe8()
   {
      return m_timeframe8;
   }
};
